...
public class PictureSelectionModel {
     ...
     * 动态设置相册启动退出动画
     *
     * @param windowAnimationStyle Activity启动退出动画主题
     * @return
     */
    ...
    public void forCustomCamera(int requestCode) {
        if (!DoubleUtils.isFastDoubleClick()) {
            Activity activity = selector.getActivity();
            if (activity == null) {
                return;
...
//            Intent intent = new Intent(activity, ProCustomCameraActivity.class);// camera2
     ...
     * Start to select media and wait for result.
     * <p>
     * 1、此方法是从forResult(int requestCode)方法复制而来，
     * 2、仅仅是把PictureSelectorActivity.class改成了PictureCustomSelectorActivity.class
     * 3、仅供城市人像app，需要设置手动扣脸和自动扣脸时使用，其他情况请使用forResult(int requestCode)方法
     *
     * @param requestCode Identity of the request Activity or Fragment.
     */
    public void forCustomResult(int requestCode) {
        if (!DoubleUtils.isFastDoubleClick()) {
            Activity activity = selector.getActivity();
            if (activity == null || selectionConfig == null) {
                return;
            ...
            Intent intent = new Intent(activity, selectionConfig.camera
                    ? PictureSelectorCameraEmptyActivity.class :
                    selectionConfig.isWeChatStyle ? PictureSelectorWeChatStyleActivity.class
                            : PictureCustomSelectorActivity.class);
            Fragment fragment = selector.getFragment();
            if (fragment != null) {
                fragment.startActivityForResult(intent, requestCode);
            } else {
     ...
     */
    @Deprecated
    private void forResult(int requestCode, int enterAnim, int exitAnim) {
        if (!DoubleUtils.isFastDoubleClick()) {
            Activity activity = selector.getActivity();
            if (activity == null) {
                return;
