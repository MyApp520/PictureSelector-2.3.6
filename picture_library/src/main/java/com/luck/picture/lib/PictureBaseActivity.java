...
public abstract class PictureBaseActivity extends AppCompatActivity implements Handler.Callback {
    ...
    protected void startCrop(String originalPath) {
        ...
        config.showCropFrame = true;// 是否显示裁剪矩形边框 圆形裁剪时建议设为false
        config.showCropGrid = true;// 是否显示裁剪矩形网格 圆形裁剪时建议设为false
        config.freeStyleCropEnabled = true;// 裁剪框是否可拖拽
        config.hideBottomControls = true;// 是否隐藏uCrop工具栏
     ...
     * @param list
     */
    protected void startCropList(ArrayList<CutInfo> list) {
        ...
        Log.e(TAG, "startCropList: 准备裁剪的文件 path = " + path);
        ...
        File desFile = new File(PictureFileUtils.getDiskCacheDir(this),
                TextUtils.isEmpty(config.renameCropFileName) ? System.currentTimeMillis() + imgType : config.renameCropFileName);

        UCropMulti.of(uri, Uri.fromFile(desFile))
                .withAspectRatio(config.aspect_ratio_x, config.aspect_ratio_y)
                .withMaxResultSize(config.cropWidth, config.cropHeight)
                .withOptions(options)
                .startAnimation(this, config.windowAnimationStyle != null
    ...
    protected void startOpenCamera() {
        ...
        cameraIntent.putExtra("android.intent.extra.quickCapture", true);//启用快捷拍照(在华为，三星手机上不起作用)
